...
static void draw_stick(int cx, int cy, int r, unsigned char ax, unsigned char ay, const char *lbl) {
    ...
    char buf[32];
    int vx = (ax - 128) * 1000 / 128;
    int vy = (ay - 128) * 1000 / 128;
    snprintf(buf, sizeof(buf), "X:%+d.%03d Y:%+d.%03d",
        vx / 1000, abs(vx) % 1000, vy / 1000, abs(vy) % 1000);
...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        ...
        fill_circle(SCREEN_W/2, 215, 18, ps_detected ? COL_PRESSED : COL_RELEASED);
        draw_text_c(SCREEN_W/2, 208, "PS", ps_detected ? 0xFF000000 : COL_TEXT, 2);
        if (ps_detected)
            draw_text_c(SCREEN_W/2, 237, "DETECTED", COL_PRESSED, 1);
        ...
        fill_rect(SCREEN_W/2 - 70, 255, 140, 30, rumble ? COL_PRESSED : COL_RELEASED);
        draw_rect(SCREEN_W/2 - 70, 255, 140, 30, rumble ? COL_PRESSED : COL_BORDER);
        draw_text_c(SCREEN_W/2, 260, "RUMBLE", rumble ? 0xFF000000 : COL_TEXT, 2);
        draw_text_c(SCREEN_W/2, 290, "Hold L1+R1", COL_TEXT, 1);
